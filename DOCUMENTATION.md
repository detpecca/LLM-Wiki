...
### 第 1 站：`llm_wiki/schema.py`（~160 行）—— 词汇表
...
- wikilink 语法 `[[dir/Page]]` 如何被解析：`WIKILINK_RE`（`schema.py:30`）和 `extract_links()`（`schema.py:37`）。

读完你应该能手写一个合法页面。这是理解一切的前提。

### 第 2 站：`llm_wiki/store.py`（~240 行）—— 文件系统层

`WikiStore` 类（`store.py:24`）封装 Wiki 目录树的所有读写。重点四个：

- `iter_pages()`（`store.py:65`）：什么算"知识页"（排除索引和 sources/）；
- `add_backlink()` / `sync_bidirectional_links()`（`store.py:123`/`:148`）：**双向链接如何维护**——A 链 B 时自动给 B 补回链，这是论文"bidirectional links"的落地；
- `rebuild_directory_index()` / `rebuild_global_index()`（`store.py:173`/`:199`）：`_index.md` 和 `index.md` 如何从磁盘页面重新生成——它们不是手写的，是**派生产物**；
- `directory_index_listing()`（`store.py:235`）：算法 1 里 SelectPages 看到的 "I" 就是这个。

### 第 3 站：`llm_wiki/validators.py`（~260 行）—— 错误检测

论文附录 F 的 7 类错误在这里变成 5 个确定性检查函数 + 1 个 LLM 验证：

- `check_dangling_links`（`:46`）、`check_incomplete_pages`（`:57`）、`check_malformed_refs`（`:70`）、`check_index_consistency`（`:91`）都是纯代码，逻辑直白，扫一眼即可；
- `check_unseen_overwrite`（`:110`）注意签名不同：它比较的是**集合**（更新的页面 ⊆ 选中的页面 ∪ 新建页面），是编译期的即时检查；`check_update`（`:118`）与之配套，在更新落盘前检查 U 内部的悬空链接和坏引用；
- `structural_validate()`（`:141`）是上面这些的聚合入口，对应算法 1 第 4 行；
- `llm_content_validate()`（`:176`）是 LLM 内容验证，注意它的 prompt（`FACT_CHECK_PROMPT`，`:153`）：逐条核对 Key Facts 是否有 digest 支撑。

### 第 4 站：`llm_wiki/error_book.py`（~160 行）—— 五阶段状态机

`ErrorBook` 类（`:38`）管理 `error_book.yaml`。对照论文 §3.3 五阶段读：

| 阶段 | 方法 | 类型 |
|---|---|---|
| 1 Discover | `discover()`（`:65`）——注意相同模式的错误会**合并计数**而非重复建条目 | 代码 |
| 2+3 Attribute/Constrain | `attribute_and_constrain()`（`:100`）——LLM 填 root_cause 和 constraint_rule | LLM |
| 4 Inject | `active_constraints()`（`:124`）——取出所有 open 条目的约束规则 | 代码 |
| 5 Verify & Close | `verify_and_close()`（`:130`）——错误消失且有约束的条目才关闭 | 代码 |

重点理解**条目字段**：phenomenon / root_cause / constraint_rule / status(open|closed) / occurrences——与论文 3.3 节逐字对应。

### 第 5 站：`llm_wiki/compile.py`（~500 行）—— 核心中的核心

`Compiler` 类（`:191`）就是算法 1。**强烈建议打开论文附录 D 的伪代码，与 `compile_passage()`（`:321`）逐行对照读**——函数里的注释标了伪代码行号：

```
伪代码行2  SelectPages        -> select_pages()      :205  (LLM)
伪代码行3  CompileWikiPages   -> compile_pages()     :220  (LLM, 注入约束)
...
伪代码行12 ApplyUpdates       -> apply_updates()     :254  (写页/写digest/补回链/重建索引)
...
- `code_autofix()`（`:233`）是第 1 层修复的确定性逻辑：丢弃悬空链接、丢弃格式错误的引用、丢弃越权更新；
- `finalize()`（`:474`）是论文 §3.3 末尾的"3 轮代码↔LLM 修复"定稿循环。

### 第 6 站：`llm_wiki/search.py`（~80 行）—— wiki_search

`search()`（`:32`）。注意权重表 `WEIGHTS`（`:13`）：页名 8 > 别名 6 > 标签 4 > 摘要 2 > 正文 1——这就是论文 §3.2"优先结构化信号，回退正文"的具体数值化。正文匹配只在结构化信号都没命中时兜底（`elif` 分支）。

### 第 7 站：`llm_wiki/agent.py`（~260 行）—— 查询期遍历

`run_agent()`（`:242`）是 ReAct 循环。对照论文图 2 读：
...
2. **索引是派生产物**：`_index.md`/`index.md` 永远可以从磁盘页面重建，所以"索引不一致"错误的修复就是直接重建（`code_fix_wiki`，`compile.py:386`）。
...
| `test_consistency.py` | 跨页矛盾检测、页对去重、抽样上限、定向 pairs 模式、Verify & Close 对 open 矛盾条目定向复查（不被抽样漏掉而假关闭） |
...
| `test_agent.py` | 桥接比较的多跳遍历、未读不许答、耐心/预算两种终止（JSON 降级路径）；native 路径同款多跳、一轮多 tool_calls、ToolsUnsupported 运行时降级；工具参数类型归一（字符串 paths / 非字符串 query） |
